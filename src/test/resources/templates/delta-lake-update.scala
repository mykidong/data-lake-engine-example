...
println("before delta table update...")
deltaTable.toDF.show()

// update delta table like UPDATE test.test_delta SET quantity = 5 WHERE itemId = 'any-item-id0'
deltaTable.updateExpr("itemId = 'any-item-id0'", Map("quantity" -> "6"))

println("after delta table update...")
deltaTable.toDF.show()

println("reading delta table with hive after delta table update...")
spark.sql("select * from test.parquet_as_delta3 where itemId = 'any-item-id0'").show()
