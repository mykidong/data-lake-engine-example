...
import mykidong.http.{OldSimpleHTTPServer, SimpleHTTPServer}
...
class OldInteractiveHandlerApplicationSpec extends FunSuite{
  ...
  test("runSparkOnLocal") {
    ...
    val port = 8125
    OldSimpleHTTPServer.run(sparkConf, propsArray, port)
    log.info("old embedded http server is running now ...")
